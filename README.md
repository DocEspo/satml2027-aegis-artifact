...
Quick Start
Create a Snowflake database and schema: `CREATE DATABASE ARTIFACT_DB; CREATE SCHEMA ARTIFACT_DB.AEGIS_REPRO;`
Execute SQL files in order: `01_schema_and_tables.sql` through `06_metric_recomputation.sql`
Load CSV data files into the corresponding tables
Run `06_metric_recomputation.sql` to independently verify all published metrics
Anonymization
